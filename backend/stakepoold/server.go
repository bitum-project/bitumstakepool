...
func runMain() error {
	...
	if err != nil || walletConn == nil {
		log.Infof("Connection to bitumwallet failed: %v", err)
		return err
	}
	log.Infof("Connected to bitumwallet (JSON-RPC API v%s)",
		walletVer.String())
	...
	if err != nil || nodeConn == nil {
		log.Infof("Connection to bitumd failed: %v", err)
		return err
	}
	...
	if err != nil {
		log.Errorf("Unable to get current network from bitumd: %v", err)
		return err
	}
	...
	for {
		...
		if err != nil {
			log.Errorf("unable to get bestblock from bitumd: %v", err)
			return err
		}
	...
	log.Info("subscribed to notifications from bitumd")
