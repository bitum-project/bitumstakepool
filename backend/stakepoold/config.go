...
var (
	defaultHomeDir     = bitumutil.AppDataDir("stakepoold", false)
...
type config struct {
	...
	BitumdHost         string   `long:"bitumdhost" description:"Hostname/IP for bitumd server"`
	BitumdUser         string   `long:"bitumduser" description:"Username for bitumd server"`
	BitumdPassword     string   `long:"bitumdpassword" description:"Password for bitumd server"`
	BitumdCert         string   `long:"bitumdcert" description:"Certificate path for bitumd server"`
	...
	RPCListeners     []string `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 9213, testnet: 19213)"`
...
func loadConfig() (*config, []string, error) {
	...
	if len(cfg.BitumdHost) == 0 {
		str := "%s: bitumdhost is not set in config"
		err := fmt.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}

	if len(cfg.BitumdCert) == 0 {
		str := "%s: bitumdcert is not set in config"
		err := fmt.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}

	if len(cfg.BitumdUser) == 0 {
		str := "%s: bitumduser is not set in config"
		err := fmt.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}

	if len(cfg.BitumdPassword) == 0 {
		str := "%s: bitumdpassword is not set in config"
		err := fmt.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}
	...
	cfg.BitumdHost = normalizeAddress(cfg.BitumdHost, activeNetParams.BitumdRPCServerPort)
	cfg.WalletHost = normalizeAddress(cfg.WalletHost, activeNetParams.WalletRPCServerPort)

	if !fileExists(cfg.BitumdCert) {
		path := filepath.Join(cfg.HomeDir, cfg.BitumdCert)
		if !fileExists(path) {
			str := "%s: bitumdcert " + cfg.BitumdCert + " and " +
				path + " don't exist"
			err := fmt.Errorf(str, funcName)
			fmt.Fprintln(os.Stderr, err)
			return nil, nil, err
