...
const (
	...
	defaultConfigFilename = "bitumstakepool.conf"
	defaultDataDirname    = "data"
	defaultLogLevel       = "info"
	defaultLogDirname     = "logs"
	defaultLogFilename    = "bitumstakepool.log"
	...
	defaultPoolFees       = 7.5
	defaultPoolLink       = "https://bitum.io"
...
var (
	bitumstakepoolHomeDir = bitumutil.AppDataDir("bitumstakepool", false)
	defaultConfigFile   = filepath.Join(bitumstakepoolHomeDir, defaultConfigFilename)
	defaultDataDir      = filepath.Join(bitumstakepoolHomeDir, defaultDataDirname)
	defaultLogDir       = filepath.Join(bitumstakepoolHomeDir, defaultLogDirname)
)
...
type config struct {
	...
	Listen             string   `long:"listen" description:"Listen for connections on the specified interface/port (default all interfaces port: 9213, testnet: 19213)"`
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(bitumstakepoolHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	funcName := "loadConfig"
	err = os.MkdirAll(bitumstakepoolHomeDir, 0700)
	if err != nil {
	...
	for idx := range cfg.WalletCerts {
		if !fileExists(cfg.WalletCerts[idx]) {
			path := filepath.Join(bitumstakepoolHomeDir, cfg.WalletCerts[idx])
			if !fileExists(path) {
	...
	if cfg.EnableStakepoold {
		...
		for idx := range cfg.StakepooldCerts {
			if !fileExists(cfg.StakepooldCerts[idx]) {
				path := filepath.Join(bitumstakepoolHomeDir,
