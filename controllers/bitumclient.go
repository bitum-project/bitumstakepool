...
type getTicketsCacheData struct {
	res   *bitumjson.TicketsForAddressResult
	timer time.Time
}

// NewGetTicketsCacheData is a contructor for getTicketsCacheData that sets the
// last get time to now.
func NewGetTicketsCacheData(tfar *bitumjson.TicketsForAddressResult) *getTicketsCacheData {
...
// missing.
func (w *walletSvrManager) fetchTransaction(txHash *chainhash.Hash) (*bitumutil.Tx,
	error) {
	var tx *bitumutil.Tx
	var err error
	for i := range w.servers {
...
// newWalletSvrManager returns a new bitum wallet server manager.
